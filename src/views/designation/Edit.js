...
import { dataDesignation, updateDesignation } from "./../../actions/designationActions";
...
class Editdesig extends Component {
  ...
  componentDidMount() {
    ...
    { 
      ...
      if (this.props.match.params){
        this.setState({ desig_id: this.props.match.params.id, token: decoded.token });
  
        const desigData = {
          desig_id: this.props.match.params.id,
          token: decoded.token
        };
  
        this.props.dataDesignation(desigData);
      }else{
  ...
  onSubmit = e => {
      e.preventDefault();
      const desigUpdateData = {
        desig_id: this.state.desig_id,
        name: this.state.name,
        token: this.state.token
      };

      this.props.updateDesignation(desigUpdateData);
  };
...
Editdesig.propTypes = {
  dataDesignation: PropTypes.func.isRequired,
...
export default connect(
  mapStateToProps,
  { dataDesignation, updateDesignation }
)(Editdesig);